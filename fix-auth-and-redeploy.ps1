Write-Host "Fixing contract auth issue and redeploying..." -ForegroundColor Cyan

$ContractFile = "contracts\event_attendance\src\lib.rs"

Write-Host "Writing fixed contract file..." -ForegroundColor Yellow
...
Set-Content -Path $ContractFile -Value $contractCode -Encoding UTF8

Write-Host "Fixed contract written successfully." -ForegroundColor Green
Write-Host "Redeploying contract..." -ForegroundColor Yellow

powershell -ExecutionPolicy Bypass -File .\scripts\deploy-contract.ps1

Write-Host "Finished. Now restart frontend with: npm run dev" -ForegroundColor Cyan
